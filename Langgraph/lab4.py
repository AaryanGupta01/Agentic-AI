# =============================================================
# LAB 4 — ReAct Agent with Tool Calling
# =============================================================
...
def weather_tool(city: str) -> str:
    ...
    """
    mock_weather_db = {
        "chennai":      {"temp": 34, "humidity": 78, "condition": "Partly Cloudy",  "wind": "18 km/h"},
        "coimbatore":   {"temp": 29, "humidity": 65, "condition": "Clear Sky",       "wind": "12 km/h"},
        "mumbai":       {"temp": 31, "humidity": 85, "condition": "Humid and Hazy",  "wind": "22 km/h"},
        "delhi":        {"temp": 28, "humidity": 55, "condition": "Sunny",           "wind": "10 km/h"},
        "bangalore":    {"temp": 25, "humidity": 60, "condition": "Light Drizzle",   "wind": "8 km/h"},
        "hyderabad":    {"temp": 32, "humidity": 58, "condition": "Mostly Sunny",    "wind": "15 km/h"},
        "london":       {"temp": 12, "humidity": 80, "condition": "Overcast",        "wind": "25 km/h"},
        "new york":     {"temp": 18, "humidity": 62, "condition": "Partly Cloudy",   "wind": "20 km/h"},
    }
...
# -------------------------------------------------------------
# STEP 3: Decision Node — LLM Picks the Right Tool
# -------------------------------------------------------------
...
1. calculator  — for any math: arithmetic, algebra, geometry formulas
2. weather     — for weather queries about a city
3. unit_converter — for converting km/miles, celsius/fahrenheit, kg/pounds
4. none        — for general questions, explanations, or greetings
...
"""
    ...
    try:
        ...
        # Fallback: if JSON parsing fails, go to direct answer
...
def direct_answer(state: AgentState) -> AgentState:
    """
    No tool needed — LLM answers directly from its own knowledge.
    """
...
# -------------------------------------------------------------
# STEP 5: Synthesis Node — LLM Wraps Tool Result into an Answer
# -------------------------------------------------------------
 
def synthesize(state: AgentState) -> AgentState:
    """
    Takes the tool result and forms a natural language response.
    Skipped if direct_answer already handled it.
    """
    if state["tool_name"] == "none":
        # direct_answer already filled tool_result with the LLM response
...
Write a clear, helpful answer to the user using the tool result.
...
# -------------------------------------------------------------
# STEP 6: Router for Conditional Edges
# -------------------------------------------------------------
...
# Conditional routing from decide_tool
...
# All tool branches converge at synthesize
