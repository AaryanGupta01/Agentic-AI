#add_conditional_edges() maps a node's output to the next node based on conditions.

# It allows for branching logic in the graph, where different paths can be taken based on the state.

# Thus, it enables the graph to handle more complex workflows with decision points.

# It helps handle scenarios where the next step depends on the output of the current node, allowing for dynamic execution paths.

#one source node can have multiple destination nodes, and the path taken can be determined by the 

# conditions defined in add_conditional_edges().
